...
pub struct CsmlStates;

impl TestableTable for CsmlStates {
    fn name() -> &'static str {
        "csml_states"
    }
...
impl ValidatableTable<&PgColumnInfo> for CsmlStates {
    fn validate(columns: &HashMap<&str, &PgColumnInfo>) {
        ...
        postgres::assert_string!(columns, "type");
...
impl ValidatableTable<&SqliteColumnInfo> for CsmlStates {
    fn validate(columns: &HashMap<&str, &SqliteColumnInfo>) {
        ...
        sqlite::assert_text!(columns, "type");
