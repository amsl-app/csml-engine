use crate::future::db_connectors::{
    conversations::{
        close_conversation, create_conversation, get_latest_open, update_conversation,
    },
    memories::internal_use_get_memories,
    state,
};
use crate::{
    data::{AsyncDatabase, ConversationInfo, EngineError},
    future::utils::{search_flow, send_msg_to_callback_url},
    init,
};

use csml_interpreter::data::context::ContextStepInfo;
use csml_interpreter::data::{
    ApiInfo, Client, Event, Message, PreviousBot,
    context::{get_hashmap_from_json, get_hashmap_from_mem},
};

use crate::data::SeaOrmDbTraits;
use crate::data::models::interpreter_actions::SwitchBot;
use crate::data::models::{BotOpt, CsmlRequest};
use crate::utils::{
    get_default_flow, get_flow_by_id, get_low_data_mode_value, get_ttl_duration_value,
};

use csml_interpreter::data::{Context, CsmlBot, CsmlFlow};

use std::collections::HashMap;
use uuid::Uuid;

/**
 * Initialize new `ConversationInfo` data, usually upon new chat request.
 * This will contain meaningful information about the request being processsed
 * and get regularly updated as the request progresses.
 *
 * This will hold references to:
 * - the bot's data,
 * - the current status of the request (steps, messages, slot, context...)
 * - the DB to use for data persistence
 * - the cached Curl connexion to the configured `callback_url`, if any
 *
 * This method takes care of the initialization of the data as well as setting up
 * some information in the database (`conversation_id`, metadata, state...).
 */
pub async fn init_conversation_info<'a, T: SeaOrmDbTraits>(
    default_flow: String,
    event: &Event,
    request: &'a CsmlRequest,
    bot: &'a CsmlBot,
    db: &mut AsyncDatabase<'_, T>,
) -> Result<ConversationInfo, EngineError> {
    tracing::info!(request_id = request.request_id, client = ?request.client, "initializing conversation info");

    // Create a new interaction. An interaction is basically each request,
    // initiated from the bot or the user.

    let mut context =
        init_context(default_flow, request.client.clone(), &bot.apps_endpoint, db).await;
    let ttl = get_ttl_duration_value(Some(event));
    let low_data = get_low_data_mode_value(event);

    // Do we have a flow matching the request? If the user is requesting a flow in one way
    // or another, this takes precedence over any previously open conversation
    // and a new conversation is created with the new flow as a starting point.
    let flow_found = search_flow(event, bot, &request.client, db).await.ok();
    let conversation_id =
        get_or_create_conversation(&mut context, bot, flow_found, &request.client, ttl, db).await?;

    context.metadata = get_hashmap_from_json(&request.metadata, &context.flow);
    context.current = get_hashmap_from_mem(
        &internal_use_get_memories(&request.client, db).await?,
        &context.flow,
    );

    let data = ConversationInfo {
        conversation_id,
        context,
        metadata: request.metadata.clone(), // ??
        request_id: request.request_id.clone(),
        callback_url: request.callback_url.clone(),
        client: request.client.clone(),
        payloads: vec![],
        ttl,
        low_data,
    };

    let flow = data.context.flow.clone();
    let step = data.context.step.clone();

    // Now that everything is correctly setup, update the conversation with wherever
    // we are now and continue with the rest of the request!
    update_conversation(data.conversation_id, Some(&flow), Some(step.get_step()), db).await?;

    Ok(data)
}

/**
 * Initialize the context object for incoming requests
 */
pub async fn init_context<T: SeaOrmDbTraits>(
    flow: String,
    client: Client,
    apps_endpoint: &Option<String>,
    db: &mut AsyncDatabase<'_, T>,
) -> Context {
    let previous_bot = get_previous_bot(&client, db).await;

    let api_info = apps_endpoint.as_ref().map(|value| ApiInfo {
        client,
        apps_endpoint: value.clone(),
    });

    Context {
        current: HashMap::new(),
        metadata: HashMap::new(),
        api_info,
        hold: None,
        step: ContextStepInfo::Normal("start".to_owned()),
        flow,
        previous_bot,
    }
}

async fn get_previous_bot<T: SeaOrmDbTraits>(
    client: &Client,
    db: &mut AsyncDatabase<'_, T>,
) -> Option<PreviousBot> {
    match state::get_state_key(client, "bot", "previous", db).await {
        Ok(Some(bot)) => serde_json::from_value(bot).ok(),
        _ => None,
    }
}

/**
 * Retrieve the current conversation, or create one if none exists.
 */
async fn get_or_create_conversation<'a, T: SeaOrmDbTraits>(
    context: &mut Context,
    bot: &'a CsmlBot,
    flow_found: Option<(&'a CsmlFlow, String)>,
    client: &Client,
    ttl: Option<chrono::Duration>,
    db: &mut AsyncDatabase<'_, T>,
) -> Result<Uuid, EngineError> {
    tracing::trace!(?client, "getting or creating conversation");
    match get_latest_open(client, db).await? {
        Some(conversation) => {
            tracing::debug!(conversation = %conversation.id, flow = conversation.flow_id, step = conversation.step_id, "conversation found");
            if let Some((flow, step)) = flow_found {
                context.step = ContextStepInfo::UnknownFlow(step);
                context.flow.clone_from(&flow.name);
            } else {
                let Ok(flow) = get_flow_by_id(&conversation.flow_id, &bot.flows) else {
                    // if flow id exist in db but not in bot close conversation
                    close_conversation(conversation.id, client, db).await?;
                    // start new conversation at default flow
                    return create_new_conversation(context, bot, flow_found, client, ttl, db)
                        .await;
                };

                context.step = ContextStepInfo::UnknownFlow(conversation.step_id.clone());
                context.flow.clone_from(&flow.name);
            }

            Ok(conversation.id)
        }
        None => create_new_conversation(context, bot, flow_found, client, ttl, db).await,
    }
}

/**
 * Create and save a new conversation in DB
 */
async fn create_new_conversation<'a, T: SeaOrmDbTraits>(
    context: &'a mut Context,
    bot: &'a CsmlBot,
    flow_found: Option<(&'a CsmlFlow, String)>,
    client: &Client,
    ttl: Option<chrono::Duration>,
    db: &mut AsyncDatabase<'_, T>,
) -> Result<Uuid, EngineError> {
    let (flow, step) = match flow_found {
        Some((flow, step)) => (flow, step),
        None => (get_default_flow(bot)?, "start".to_owned()),
    };

    let conversation_id = create_conversation(&flow.id, &step, client, ttl, db).await?;

    context.step = ContextStepInfo::UnknownFlow(step);
    context.flow.clone_from(&flow.name);

    Ok(conversation_id)
}

/**
 * Switch bot find next bot in DB and create new Client and new conversation
 */
pub async fn switch_bot<T: SeaOrmDbTraits>(
    data: &mut ConversationInfo,
    bot: &mut CsmlBot,
    next_bot: SwitchBot,
    bot_opt: &mut BotOpt,
    event: &mut Event,
    db: &mut AsyncDatabase<'_, T>,
) -> Result<(), EngineError> {
    tracing::debug!(
        current_bot_id = bot.id,
        next_bot_id = next_bot.bot_id,
        "switching bot"
    );
    // update data info with new bot |ex| client bot_id, create new conversation
    *bot_opt = match next_bot.version_id {
        Some(version_id) => BotOpt::Id {
            version_id,
            bot_id: next_bot.bot_id,
            apps_endpoint: bot.apps_endpoint.take(),
            multibot: bot.multibot.take(),
        },
        None => BotOpt::BotId {
            bot_id: next_bot.bot_id,
            apps_endpoint: bot.apps_endpoint.take(),
            multibot: bot.multibot.take(),
        },
    };

    let mut new_bot = bot_opt.search_bot_async(db).await?;
    new_bot.custom_components = bot.custom_components.take();
    new_bot.native_components = bot.native_components.take();

    *bot = new_bot;

    init::set_bot_ast(bot)?;

    data.context.step = ContextStepInfo::UnknownFlow(next_bot.step);
    data.context.flow = match next_bot.flow {
        Some(flow) => flow,
        None => bot.get_default_flow_name(),
    };

    // update client with the new bot id
    data.client.bot_id.clone_from(&bot.id);

    let (flow, step) = if let Ok(flow) = get_flow_by_id(&data.context.flow, &bot.flows) {
        (flow, data.context.step.clone())
    } else {
        let error_message = format!(
            "flow: [{}] not found in bot: [{}], switching to start@default_flow",
            data.context.flow, bot.name
        );

        let message = Message {
            content_type: "error".to_owned(),
            content: serde_json::json!({ "error": error_message }),
        };

        // save message
        data.payloads.push(message.clone());
        // send message
        send_msg_to_callback_url(data, vec![message], 0).await?;

        // setting default step && flow
        data.context.step = ContextStepInfo::Normal("start".to_owned());
        data.context.flow = bot.get_default_flow_name();

        (
            get_flow_by_id(&bot.default_flow, &bot.flows)?,
            ContextStepInfo::Normal("start".to_owned()),
        )
    };

    // update event to flow trigger
    "flow_trigger".clone_into(&mut event.content_type);
    event.content = serde_json::json!({
            "flow_id": flow.id,
            "step_id": step
        }
    );

    // create new conversation for the new client
    data.conversation_id =
        create_conversation(&flow.id, step.get_step(), &data.client, data.ttl, db).await?;

    // and get memories of the new bot form db,
    // clearing the permanent memories form scope of the previous bot
    data.context.current = get_hashmap_from_mem(
        &internal_use_get_memories(&data.client, db).await?,
        &data.context.flow,
    );

    Ok(())
}
